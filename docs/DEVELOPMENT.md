...
- Varsayılan sağlayıcı OpenAI Responses API; varsayılan model: `gpt-4.1-mini`
...
Faz 0/1 backend, Faz 2 Outlook add-in, yanıt taslağı ve Windows otomatik başlangıç kurulumu tamamlandı:
...
## Sınırlı mail bağlamı

Analiz ve taslak aynı yerel temizleme akışını kullanır. Güncel mesaj önce korunur;
en yakın iki geçmiş mesaj için toplam en fazla 4.000 karakter ayrılır. Başlıklar
ve ayırıcılarla toplam mail bağlamı 16.000 karakteri aşmaz. Bu bir token limiti
değildir; sistem promptu ve taslak talimatları ayrıca eklenir. Ek özetleme çağrısı yapılmaz.

Outlook Türkçe/İngilizce yanıt başlıkları birlikte değerlendirilir. Açık imzalar,
tam tekrar mesajlar ve tekrarlanan bilinen gizlilik altbilgileri ayıklanır.
Belirsiz sınırlar veya satır arası yanıtlar tahminle silinmez; bütün metin bütçeye
göre korunur. İki mesajdan eski konuşmaya bağlı bilgiler analize girmeyebilir.

Analiz ve taslak yanıtındaki `contentProcessing` alanı önceki/sonraki karakter
sayısını, dahil edilen geçmiş mesaj sayısını, geçmiş sınırlandırmasını, güncel
mesajın kırpılmasını ve ayrıştırma belirsizliğini bildirir. Normal geçmiş
sadeleştirmesi bilgi notudur; güncel mesaj kırpılırsa veya belirsiz ayrıştırmada
içerik dışarıda kalırsa sarı uyarı gösterilir.
...
- .NET 8 SDK
- OpenAI için API anahtarı; alternatif olarak Ollama ve `qwen3.5:4b` modeli
...
Ollama'ya geri dönmek için API'yi durdurup sağlayıcıyı açıkça seçin:
